...
public class BuildReference {
    ...
    private static Set<Point> pointsClicked = new HashSet<>();
        ...
        applyMovementsButton.addActionListener(listener -> {
            Runnable task =
                () -> {
                    ...
                    System.out.println("movements: " + movementsArray.length);
                    System.out.println("residue list: " + residuesList.size());
    ...
    static class MyCanvas extends JPanel implements MouseListener {
        ...
        public void paint(Graphics g) {
            ...
            // }
            // int residueX = sizeFactor * (residues.get(i).getPoint().getX()) +
            // sizeFactor / 4;
            // int residueY = sizeFactor * (residues.get(i).getPoint().getY()) +
            // sizeFactor / 4;
            // if (residues.get(i).getResidueType().equals(ResidueType.H)) {
            // g.setColor(Color.BLACK);
            // g.fillArc(residueX, residueY, sizeFactor / 2, sizeFactor / 2, 0,
            // 360);
            // } else {
            ...
            //
            // if (next != null) {
            // int nextX = sizeFactor * (next.getPoint().getX()) + sizeFactor /
            // 4;
            // int nextY = sizeFactor * (next.getPoint().getY()) + sizeFactor /
            // 4;
            // g.drawLine(residueX + sizeFactor / 4, residueY + sizeFactor / 4,
            // nextX + sizeFactor / 4, nextY
            // + sizeFactor / 4);
            // }
            //
            // }
            if (point != null) {
                g.fillArc(sizeFactor * point.getX() + sizeFactor / 4, sizeFactor * point.getY() + sizeFactor / 4,
                    sizeFactor / 2, sizeFactor / 2, 0, 360);
                if (pointsClicked.size() == 100) {
                    JOptionPane.showMessageDialog(this, "ACABOU ACABOU ACABOU É TRETA TRETA TRETA");
                }
            }
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            point = new Point(x, y);
            boolean add = pointsClicked.add(point);
            if (add) {
                // System.out.println(x + "," + y);
                System.out.println("residues.add(new Residue(new Point(" + point.getX() + ", " + point.getY()
                    + "), ResidueType.valueOf(String.valueOf(chain.charAt(" + pointsClicked.size() + ")))))");
            }
